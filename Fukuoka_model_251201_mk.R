...
emp_j_data<-emp_j%>%
  tibble::column_to_rownames(var="KEY_CODE")
L_j_hat<-t(emp_j_data)

# sum(emp_j$T000841002)
# sum(L_j_hat)
...
zone_population<-left_join(zone_population,bnd_mesh_wL10_filterd,by="KEY_CODE")%>%
  dplyr::select(-"T000841002")
...
household<-bnd_mesh_wL10_filterd%>%
  left_join(household,by="KEY_CODE")%>%
  dplyr::mutate(household=tidyr::replace_na(household,0))%>%
  dplyr::select(-T000841002)
